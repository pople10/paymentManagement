...
class loseService
{
	...
	{
		$query="SELECT * FROM lose WHERE id=?";
	...
	{
		$query="INSERT INTO lose (amount,type,date,currency,warrently,target) VALUES (?,?,?,?,?,?)";
	...
	{
		$query="SELECT amount FROM lose WHERE id=?";
		...
		$query="UPDATE lose SET amount=?,currency=?,warrently=?,target=?,type=?,date=? WHERE id=?";
	...
	public function deleteit($id)
	{
		...
		$query="DELETE FROM lose WHERE id=?";
	...
	{
		$query="SELECT SUM(amount) as amount,month(date) as month FROM lose WHERE year(date)=year(NOW()) AND currency='USD' GROUP BY month(date)";
	...
	{
		$query="SELECT SUM(amount) as amount,month(date) as month FROM lose WHERE year(date)=year(NOW()) AND currency='MAD' GROUP BY month(date)";
	...
	{
		$query="SELECT SUM(amount) as amount,year(date) as year FROM lose WHERE currency='USD' GROUP BY year(date) LIMIT 10";
	...
	{
		$query="SELECT SUM(amount) as amount,year(date) as year FROM lose WHERE currency='MAD' GROUP BY year(date) LIMIT 10";
