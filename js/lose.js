...
$("a[href='lose']").addClass("active");
function update(id)
{
	var data = $.ajax({url:"controller/loseController.php",type:"POST",data:{op:"findById",id:id},async:false}).responseText;
	...
	$("#submit").click(function(){
		...
		if(err===null)
			$.ajax({
				url:"controller/loseController.php",
				type:"POST",
				data:{op:"add",amount:amount,type:type,date:date,currency:currency,warrently:warrently,target:target},
				async:false,
				success:function()
				{
				...
				error: function(xhr, status, error) {
					alert(xhr.responseText);
					alertify.error("Something wrong try alose");
					console.log(xhr + " - " + status + " - " + error);
		...
		{	
			...
			$.ajax({
				url:"controller/loseController.php",
				type:"POST",
				data:{op:"update",amount:amount,type:type,date:date,currency:currency,warrently:warrently,target:target,id:id},
				async:false,
				success:function(data)
				{	
